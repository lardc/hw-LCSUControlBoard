// -----------------------------------------
// Global definitions
// ----------------------------------------

// Header
#include "Constraints.h"

// Includes
#include "Regulator.h"

// Constants
//
const TableItemConstraint Constraint[DATA_TABLE_WP_START] = {
		{CURRENT_THRESHOLD_MIN, CURRENT_THRESHOLD_MAX_HIGH, CURRENT_THRESHOLD_DEF_HIGH},		// 0
		{V_BAT_THRESHOLD_MIN, V_BAT_THRESHOLD_MAX, V_BAT_THRESHOLD_DEF},						// 1
		{DAC_OUTPUT_LIM_MIN, DAC_OUTPUT_LIM_MAX, DAC_OUTPUT_LIM_DEF},							// 2
		{FULL_CHARGE_TIMEOUT_MIN, FULL_CHARGE_TIMEOUT_MAX, FULL_CHARGE_TIMEOUT_DEF},			// 3
		{RECHARGE_TIMEOUT_MIN, RECHARGE_TIMEOUT_MAX, RECHARGE_TIMEOUT_DEF},						// 4
		{SHUNT_RESISTANCE_MIN, SHUNT_RESISTANCE_MAX, SHUNT_RESISTANCE_DEF},						// 5
		{0, INT16U_MAX, 0},																		// 6
		{NO, YES, YES},																			// 7
		{NO, YES, YES},																			// 8
		{0, 0, 0},																				// 9
		{I_PER_CURBOARD_MIN, I_PER_CURBOARD_MAX, I_PER_CURBOARD_DEF},							// 10
		{CURBOARDS_MIN, CURBOARDS_MAX, CURBOARDS_DEF},											// 11
		{PULSE_PAUSE_AVG_CURR_MIN, PULSE_PAUSE_AVG_CURR_MAX, PULSE_PAUSE_AVG_CURR_DEF},			// 12
		{FAN_PERIOD_MIN, FAN_PERIOD_MAX, FAN_PERIOD_DEF},										// 13
		{FAN_TIME_MIN, FAN_TIME_MAX, FAN_TIME_DEF},												// 14
		{0, 0, 0},																				// 15
		{CONF_STATE_TIMEOUT_MIN, CONF_STATE_TIMEOUT_MAX, CONF_STATE_TIMEOUT_DEF},				// 16
		{TRAPEZE_CUR_RATE_MIN, TRAPEZE_CUR_RATE_MAX, TRAPEZE_CUR_RATE_DEF},						// 17
		{0, 0, 0},																				// 18
		{PSH_Sine, PSH_Trapeze, PSH_Sine},														// 19
		{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},												// 20
		{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},												// 21
		{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},												// 22
		{COEF_K_MIN, COEF_K_MAX, COEF_K_DEF},													// 23
		{OFFSET_MIN, OFFSET_MAX, OFFSET_DEF},													// 24
		{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},												// 25
		{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},												// 26
		{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},												// 27
		{COEF_K_MIN, COEF_K_MAX, COEF_K_DEF},													// 28
		{OFFSET_MIN, OFFSET_MAX, OFFSET_DEF},													// 29
		{V_BAT_K_MIN, V_BAT_K_MAX, V_BAT_K_DEF},												// 30
		{OFFSET_MIN, OFFSET_MAX, OFFSET_DEF},													// 31
		{-INT16S_MAX, INT16S_MAX, 0},															// 32
		{-INT16S_MAX, INT16S_MAX, 0},															// 33
		{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},												// 34
		{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},												// 35
		{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},												// 36
		{COEF_K_MIN, COEF_K_MAX, COEF_K_DEF},													// 37
		{OFFSET_MIN, OFFSET_MAX, OFFSET_DEF},													// 38
		{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},												// 39
		{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},												// 40
		{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},												// 41
		{COEF_K_MIN, COEF_K_MAX, COEF_K_DEF},													// 42
		{OFFSET_MIN, OFFSET_MAX, OFFSET_DEF},													// 43
		{REGULATOR_KP_MIN, REGULATOR_KP_MAX, REGULATOR_KP_DEF},									// 44
		{REGULATOR_KI_MIN, REGULATOR_KI_MAX, REGULATOR_KI_DEF},									// 45
		{REGULATOR_KP_MIN, REGULATOR_KP_MAX, REGULATOR_KP_DEF},									// 46
		{REGULATOR_KI_MIN, REGULATOR_KI_MAX, REGULATOR_KI_DEF},									// 47
		{0, 0, 0},																				// 48
		{0, 0, 0},																				// 49
		{0, 0, 0},																				// 50
		{ALLOWED_ERR_MIN, ALLOWED_ERR_MAX, ALLOWED_ERR_DEF},									// 51
		{FOLLOWING_ERR_CNT_MIN,FOLLOWING_ERR_CNT_MAX,FOLLOWING_ERR_CNT_DEF},					// 52
		{NO, YES, NO},																			// 53
		{REF_VOLTAGE_MIN, REF_VOLTAGE_MAX, REF_VOLTAGE_DEF},									// 54
		{0, 0, 0},																				// 55
		{0, 0, 0},																				// 56
		{0, 0, 0},																				// 57
		{0, 0, 0},																				// 58
		{0, 0, 0},																				// 59
		{0, INT16U_MAX, 0},																		// 60
		{0, CURRENT_SETPOINT_MAX, 0},															// 61
		{0, CURRENT_SETPOINT_MAX, 0},															// 62
		{CURRENT_THRESHOLD_MIN, CURRENT_THRESHOLD_MAX_LOW, CURRENT_THRESHOLD_DEF_LOW},			// 63
		{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},												// 64
		{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},												// 65
		{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},												// 66
		{COEF_K_MIN, COEF_K_MAX, COEF_K_DEF},													// 67
		{OFFSET_MIN, OFFSET_MAX, OFFSET_DEF},													// 68
		{0, 0, 0},																				// 69
		{REGULATOR_KP_MIN, REGULATOR_KP_MAX, REGULATOR_KP_DEF},									// 70
		{REGULATOR_KI_MIN, REGULATOR_KI_MAX, REGULATOR_KI_DEF},									// 71
		{0, 0, 0},																				// 72
		{0, 0, 0},																				// 73
		{0, 0, 0},																				// 74
		{0, 0, 0},																				// 75
		{0, 0, 0},																				// 76
		{0, 0, 0},																				// 77
		{0, 0, 0},																				// 78
		{0, 0, 0},																				// 79
		{0, 0, 0},																				// 80
		{0, 0, 0},																				// 81
		{0, 0, 0},																				// 82
		{0, 0, 0},																				// 83
		{0, 0, 0},																				// 84
		{0, 0, 0},																				// 85
		{0, 0, 0},																				// 86
		{0, 0, 0},																				// 87
		{0, 0, 0},																				// 88
		{0, 0, 0},																				// 89
		{0, 0, 0},																				// 90
		{0, 0, 0},																				// 91
		{0, 0, 0},																				// 92
		{0, 0, 0},																				// 93
		{0, 0, 0},																	// 94
		{0, 0, 0},																	// 95
		{0, 0, 0},																	// 96
		{0, 0, 0},																	// 97
		{0, 0, 0},																	// 98
		{0, 0, 0},																	// 99
		{0, 0, 0},																	// 100
		{0, 0, 0},																	// 101
		{0, 0, 0},																	// 102
		{0, 0, 0},																	// 103
		{0, 0, 0},																	// 104
		{0, 0, 0},																	// 105
		{0, 0, 0},																	// 106
		{0, 0, 0},																	// 107
		{0, 0, 0},																	// 108
		{0, 0, 0},																	// 109
		{0, 0, 0},																	// 110
		{0, 0, 0},																	// 111
		{0, 0, 0},																	// 112
		{0, 0, 0},																	// 113
		{0, 0, 0},																	// 114
		{0, 0, 0},																	// 115
		{0, 0, 0},																	// 116
		{0, 0, 0},																	// 117
		{0, 0, 0},																	// 118
		{0, 0, 0},																	// 119
		{PCB_VERSION_10, PCB_VERSION_11, PCB_VERSION_10},							// 120
		{0, 0, 0},																	// 121
		{0, 0, 0},																	// 122
		{0, 0, 0},																	// 123
		{0, 0, 0},																	// 124
		{0, 0, 0},																	// 125
		{0, 0, 0},																	// 126
		{0, 0, 0},																	// 127
		{CURRENT_SETPOINT_MIN, CURRENT_SETPOINT_MAX, 0},							// 128
		{TRAPEZE_DURATION_MIN, TRAPEZE_DURATION_MAX, TRAPEZE_DURATION_DEF},			// 129
		{0, 0, 0},																	// 130
		{0, 0, 0},																	// 131
		{0, 0, 0},																	// 132
		{0, 0, 0},																	// 133
		{0, 0, 0},																	// 134
		{0, 0, 0},																	// 135
		{0, 0, 0},																	// 136
		{0, 0, 0},																	// 137
		{0, 0, 0},																	// 138
		{0, 0, 0},																	// 139
		{0, 0, 0},																	// 140
		{0, 0, 0},																	// 141
		{0, 0, 0},																	// 142
		{0, 0, 0},																	// 143
		{0, 0, 0},																	// 144
		{0, 0, 0},																	// 145
		{0, 0, 0},																	// 146
		{0, 0, 0},																	// 147
		{0, 0, 0},																	// 148
		{0, 0, 0},																	// 149
		{-INT16S_MAX, INT16S_MAX, 0},												// 150
		{NO, YES, NO},																// 151
		{NO, YES, NO},																// 152
		{0, 0, 0},																	// 153
		{0, 0, 0},																	// 154
		{0, 0, 0},																	// 155
		{0, 0, 0},																	// 156
		{0, 0, 0},																	// 157
		{0, 0, 0},																	// 158
		{0, 0, 0},																	// 159
		{0, 0, 0},																	// 160
		{0, 0, 0},																	// 161
		{0, 0, 0},																	// 162
		{0, 0, 0},																	// 163
		{0, 0, 0},																	// 164
		{0, 0, 0},																	// 165
		{0, 0, 0},																	// 166
		{0, 0, 0},																	// 167
		{0, 0, 0},																	// 168
		{0, 0, 0},																	// 169
		{0, 0, 0},																	// 170
		{0, 0, 0},																	// 171
		{0, 0, 0},																	// 172
		{0, 0, 0},																	// 173
		{0, 0, 0},																	// 174
		{0, 0, 0},																	// 175
		{0, 0, 0},																	// 176
		{0, 0, 0},																	// 177
		{0, 0, 0},																	// 178
		{0, 0, 0},																	// 179
		{0, 0, 0},																	// 180
		{0, 0, 0},																	// 181
		{0, 0, 0},																	// 182
		{0, 0, 0},																	// 183
		{0, 0, 0},																	// 184
		{0, 0, 0},																	// 185
		{0, 0, 0},																	// 186
		{0, 0, 0},																	// 187
		{0, 0, 0},																	// 188
		{0, 0, 0},																	// 189
		{0, 0, 0},																	// 190
		{0, 0, 0}																	// 191
};
